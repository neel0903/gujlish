// JS port vs the Python reference, plus the JS-only features.
//     python build_site.py && node web/test_port.js
"use strict";
const fs = require("fs");
...
const GUJLISH_DATA = loadVar("lexicon.js");
const ENGLISH = loadVar("english.js");

let bad = 0;
function check(label, cond) { if (!cond) { bad++; console.log("FAIL " + label); } else console.log("ok   " + label); }

// 1. Phonetic keys: every lexicon word, prefix and whole-word mode, and
//    the keys precomputed into lexicon.js must be the JS ones.
let keyBad = 0;
...
bad += keyBad;
let preBad = 0;
for (const w of GUJLISH_DATA.words) if (w[2] !== G.strictKey(w[0], true) || w[3] !== G.looseKey(w[0], true)) preBad++;
check(`precomputed keys in lexicon.js match (${preBad} mismatches)`, preBad === 0);

// 2. Engine trials.
const t0 = Date.now();
const engine = new G.Engine(GUJLISH_DATA.words, GUJLISH_DATA.bigrams, null, GUJLISH_DATA.trigrams);
console.log(`engine built in ${Date.now() - t0} ms`);
let trialBad = 0;
for (const { typed, prev, prev2, result } of expected.trials) {
  const got = typed ? engine.suggest(typed, prev, prev2) : engine.nextWord(prev, prev2);
  const same = got.join(",") === result.join(",");
  if (!same) {
    trialBad++;
    const label = (prev ? `[${prev2 ? prev2 + " " : ""}${prev}] ` : "") + JSON.stringify(typed);
    console.log(`DIFF ${label.padEnd(20)} js: ${got.join(", ")}   py: ${result.join(", ")}`);
  }
}
console.log(`${expected.trials.length - trialBad}/${expected.trials.length} trials identical`);
bad += trialBad;

// 2b. Autocorrect, against the Python reference.
let corrBad = 0;
for (const { typed, prev, prev2, result } of expected.corrections) {
  const got = engine.correct(typed, prev, prev2);
...
const mixed = new G.Engine(GUJLISH_DATA.words, GUJLISH_DATA.bigrams, ENGLISH, GUJLISH_DATA.trigrams);
check("mixed: 'kem' shows no English", Object.values(mixed.suggestDetailed("kem").sources).every(v => v !== "en"));
...
mixed.learnTrigram("kem", "cho", "neelbhai", 4);
check("personal trigram predicts (" + mixed.nextWord("cho", "kem").join(",") + ")", mixed.nextWord("cho", "kem")[0] === "neelbhai");
check("trigram needs both words (" + mixed.nextWord("cho").join(",") + ")", mixed.nextWord("cho").indexOf("neelbhai") < 0);
...
check("forget restores (" + mixed.suggest("th", "kem").join(",") + ")", mixed.suggest("th", "kem").join(",") === engine.suggest("th", "kem").join(",") && !mixed.bySurface["neelbhai"] && mixed.nextWord("cho", "kem")[0] !== "neelbhai");

// The user's own spec for autocorrect.
let prev = null, prev2 = null, fixed = [];
for (const w of ["Avi", "gaye", "ghara"]) { const f = mixed.correct(w, prev, prev2); fixed.push(f || w); prev2 = prev; prev = f || w; }
...
for (let rep = 0; rep < 3; rep++) {
  for (const p of probes) {
    ...
    mixed.suggest(p, "tame", "aaje");
    const ms = Number(process.hrtime.bigint() - t) / 1e6;
    if (ms > worst) worst = ms;
  }
}
let worstCorrect = 0;
for (const p of ["ghara", "gaye", "thayoo", "sarkr", "meeting"]) {
  const t = process.hrtime.bigint();
  mixed.correct(p, "gaya", "aavi");
  worstCorrect = Math.max(worstCorrect, Number(process.hrtime.bigint() - t) / 1e6);
}
console.log(`worst suggest latency ${worst.toFixed(1)} ms, worst correct latency ${worstCorrect.toFixed(1)} ms`);
